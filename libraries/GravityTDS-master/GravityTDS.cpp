...
void GravityTDS::setEcValue(float rawECsolution, float temperature)
{
      float ecSolution;
      ecSolution = rawECsolution*(1.0+0.02*(temperature-25.0));
      kValue = rawECsolution/(133.42*voltage*voltage*voltage - 255.86*voltage*voltage + 857.39*voltage);  //calibrate in the  buffer solution, such as 707ppm(1413us/cm)@25^c
...
void GravityTDS::ecCalibration(byte mode)
{
    ...
    switch(mode)
    {
      ...
      if(enterCalibrationFlag)
      {
          ...
          if((rawECsolution>0) && (rawECsolution<2000) && (KValueTemp>0.25) && (KValueTemp<10.0))
          {
              Serial.println();
